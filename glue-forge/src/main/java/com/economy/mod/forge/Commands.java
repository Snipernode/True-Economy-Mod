...
public final class Commands {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(net.minecraft.commands.Commands.literal("economy")
                ...
                .then(net.minecraft.commands.Commands.argument("args", com.mojang.brigadier.arguments.StringArgumentType.greedyString())
                        .executes(ctx -> exec(ctx, com.mojang.brigadier.arguments.StringArgumentType.getString(ctx, "args").trim()))));
    }

    private static int exec(CommandContext<CommandSourceStack> ctx, String rest) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        ServerPlayer player = ctx.getSource().getPlayerOrException();
        run(player, rest.isEmpty() ? new String[0] : rest.split("\\s+"));
        return 1;
    }
    ...
    public static void run(ServerPlayer player, String[] args) {
        ...
        String owner = TrueEconomyMod.platform.playerName(player);
    ...
    // ---- helpers ----
    static void reply(ServerPlayer p, String msg) { TrueEconomyMod.platform.send(p, msg); }
    static void error(ServerPlayer p, String msg) { TrueEconomyMod.platform.send(p, "[!] " + msg); }
    ...
    private static void auction(ServerPlayer p, String owner, String[] args) {
        ...
        switch (args[1].toLowerCase(Locale.ROOT)) {
            ...
            case "bid":
                ...
                Auction target = ah.get(UUID_FROM_STRING(args[2]));
            ...
            case "buy":
                ...
                Auction b = ah.get(UUID_FROM_STRING(args[2]));
            ...
            case "claim":
                ...
                Auction c = ah.get(UUID_FROM_STRING(args[2]));
    ...
    private static java.util.UUID UUID_FROM_STRING(String s) {
    ...
    private static void buyBlackMarket(ServerPlayer p, EconomyCore core, String canon, String sectionId, String itemId) {
        for (ShopSection s : core.catalog().all()) {
            ...
            if (!core.economy().getAccount(TrueEconomyMod.platform.playerName(p)).withdraw(total)) { error(p, "Insufficient funds."); return; }
